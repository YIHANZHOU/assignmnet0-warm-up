...
float g_mouse_down = false;   //screen information
float g_clicked_x = -1;
float g_clicked_y = -1;
...
//    }
        ...
        g_bScale = true;
        if(((x < 1.02 && x > .98)||(x >-1.02 && x <- .98)) &&((y < 1.02 && y > .98)||(y >-1.02 && y <- .98))){
            g_bRotate=true;
        }
        printf("scale");
    }
...
void mouseDragged(float m_x, float m_y){
    ...
    if (g_bScale){
        ...
        float center_x=1/2*(vertices[0]+vertices[21]); //the center of the square equals the average x of the top right and bottom left point
        float center_y=1/2*(vertices[1]+vertices[22]);//the center of the square equals the average y of the top right and bottom left point
        if((m_x-center_x)*(m_x-center_x)+(m_y-center_y)*(m_y-center_y)>(g_clicked_x-center_x)*(g_clicked_x-center_x)+(g_clicked_y-center_y)*(g_clicked_y-center_y)){g_size=g_size+0.0006f;}
        else{g_size=g_size-0.0006f;}
    ...
    if (g_bRotate){
        float center_x=1/2*(vertices[0]+vertices[21]); //the center of the square equals the average x of the top right and bottom left point
        float center_y=1/2*(vertices[1]+vertices[22]);//the center of the square equals the average y of the top right and bottom left point
        float A_x=m_x-center_x;
        float A_y=m_y-center_y;
        float B_x=g_clicked_x-center_x;
        float B_y=g_clicked_y-center_y;
        float g_angle=(atan2(A_y,A_x)-atan2(B_x,B_y));
        printf("g_angle:%s\n",glGetString(g_angle));
